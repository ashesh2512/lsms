...
void lsms::nrel_in(Real energy, int l_qn, Real *p_array, Real *q_array,
  ...
  Real val0[2], val1[2], val2[2], val3[2];

  Real delta;
  Real chi, inv0, inv1, step_val1, step_val3, step_val2;

#if SP
  constexpr auto tolerance = FLT_MIN;
#else
  constexpr auto tolerance = DBL_MIN;
#endif

  Real omega = std::sqrt(-2 * energy);
  ...
  inv0 = p_array[idx - 2] - toReal(1.0) / 24 * (val0[0] - 5 * val1[0] + 19 * val2[0]);
  inv1 = q_array[idx - 2] - toReal(1.0) / 24 * (val0[1] - 5 * val1[1] + 19 * val2[1]);
  chi = -9.0 / 24.0;

  step_val1 = r_mesh[idx - 3];
  step_val2 = h * r_mesh[idx - 3];
  step_val3 = -2 * h * r_mesh[idx - 3] * (energy - pot[idx - 3] - l_qn * (l_qn + 1) / (2 * step_val1 * step_val1));
  ...
  for (idx = start_idx; idx > stop + 4; --idx) {
    ...
    inv0 = p_array[idx - 3] -
        toReal(1.0 / 720) *
            (-19 * val0[0] + 106 * val1[0] - 264 * val2[0] + 646 * val3[0]);

    inv1 = q_array[idx - 3] -
        toReal(1.0 / 720) *
            (-19 * val0[1] + 106 * val1[1] - 264 * val2[1] + 646 * val3[1]);
    ...
    step_val3 = -2 * h * r_mesh[idx - 4] * (energy - pot[idx - 4] - l_qn * (l_qn + 1) / (2 * step_val1 * step_val1));
    delta = 1 - chi * chi * step_val2 * step_val3;
...
void lsms::nrel_out(Real Z, Real energy, int l_qn, Real *p_array, Real *q_array,
                    const Real *__restrict__ r_mesh,
                    Real h,
                    const Real *__restrict__ pot, std::size_t end,
                    int sign_switch) {
  auto idx = end - end;
  Real yp[2], val0[2], val1[2], val2[2], val3[2];
  Real delta, step_val3, chi, step_val1, step_val2, inv0, inv1;

  if (l_qn == 0) {
    if (Z <= L_LIM) {
      p_array[0] = (2 + 2 * energy * r_mesh[0] * r_mesh[0]) * pow_t(toReal(-1), sign_switch);
      q_array[0] = energy * r_mesh[0] * pow_t(toReal(-1), sign_switch);
    } else {
      p_array[0] = (r_mesh[0] - Z * r_mesh[0] * r_mesh[0]) * pow_t(toReal(-1), sign_switch);
      q_array[0] = (1 - 2 * r_mesh[0] * Z) * pow_t(toReal(-1), sign_switch);
    }
  } else {
    p_array[0] = pow_t(r_mesh[0], (l_qn + 1)) * pow_t(toReal(-1), sign_switch);
    q_array[0] = (l_qn + 1) * pow_t(r_mesh[0], l_qn) * pow_t(toReal(-1), sign_switch);
  }
  ...
  inv0 = p_array[1] + toReal(1.0 / 12.0) * (-val0[0] + 8 * val1[0]);
  inv1 = q_array[1] + toReal(1.0 / 12.0) * (-val0[1] + 8 * val1[1]);

  chi = 5.0 / 12.0;
  step_val3 = -2 * h * r_mesh[2] * (energy - pot[2] - l_qn * (l_qn + 1) / (2 * r_mesh[2] * r_mesh[2]));
  ...
  inv0 = p_array[2] + toReal(1.0) / 24 * (val0[0] - 5 * val1[0] + 19 * val2[0]);
  inv1 = q_array[2] + toReal(1.0) / 24 * (val0[1] - 5 * val1[1] + 19 * val2[1]);
  ...
  step_val3 = -2 * h * r_mesh[3] * (energy - pot[3] - l_qn * (l_qn + 1) / (2 * step_val1 * step_val1));
  delta = 1 - chi * chi * step_val2 * step_val3;
  ...
  for (idx = 3; idx < end - 1; ++idx) {
    ...
    step_val3 = -2 * h * r_mesh[idx + 1] * (energy - pot[idx + 1] - l_qn * (l_qn + 1) / (2 * step_val1 * step_val1));
...
inline void lsms::rel_in(Real energy, int k_qn, Real *p_array, Real *q_array,
                         const Real *__restrict__ r_mesh,
                         Real h,
                         const Real *__restrict__ pot, std::size_t end,
                         Real &p_last, Real &q_last, std::size_t stop,
                         std::size_t &imax) {
  auto idx = end - 1;
  auto start_idx = idx;

  Real val0[2], val1[2], val2[2];
  Real delta, chi, inv0, inv1;
  ...
  Real omega = std::sqrt(-2 * energy - energy * energy / (C_SQ));
  ...
  inv0 = p_array[idx - 2] - toReal(1.0 / 24.0) * (val0[0] - 5 * val1[0] + 19 * val2[0]);
  inv1 = q_array[idx - 2] - toReal(1.0 / 24.0) * (val0[1] - 5 * val1[1] + 19 * val2[1]);
...
inline void lsms::rel_out(Real energy, int k_qn, Real *p_array, Real *q_array,
                          const Real *__restrict__ r_mesh,
                          Real h,
                          const Real *__restrict__ pot, std::size_t end,
                          int sign_switch) {
  Real yp[2], val0[2], val1[2], val2[2], val3[3];
  Real delta, chi, inv0, inv1, Mat11, Mat21, Mat12, Mat22;

  Real Z = -pot[0] * r_mesh[0];
  Real beta = std::sqrt(k_qn * k_qn - (Z * Z) / (C * C));

  p_array[0] = std::pow(r_mesh[0], beta);
  q_array[0] = std::pow(r_mesh[0], beta) * C * (beta + k_qn) / Z;
  p_array[0] = p_array[0] * pow_t(toReal(-1), sign_switch);
  q_array[0] = q_array[0] * pow_t(toReal(-1), sign_switch);
  ...
  inv0 = p_array[1] + (-val0[0] + 8 * val1[0]) / 12;
  inv1 = q_array[1] + (-val0[1] + 8 * val1[1]) / 12;
  ...
  inv0 = p_array[2] + (val0[0] - 5 * val1[0] + 19 * val2[0]) / 24;
  inv1 = q_array[2] + (val0[1] - 5 * val1[1] + 19 * val2[1]) / 24;
...
inline Real lsms::rel_integration(const Real *__restrict__ p_array,
                                    const Real *__restrict__ q_array,
                                    const Real *__restrict__ r_mesh,
                                    Real h, std::size_t end) {
  Real cummulative_val = 0.0;
  ...
  if ((end % 2) == 0) {
    cummulative_val += toReal(0.5) * ((p_array[idx] * p_array[idx] + q_array[idx] * q_array[idx]) * h * r_mesh[idx] +
...
inline Real lsms::nrel_integration(const Real *__restrict__ P,
                                     const Real *__restrict__ r_mesh,
                                     Real h,
                                     std::size_t end) {
  Real cummulative_val = 0.0;
  ...
  for (idx = 0; idx < end - 2; idx += 2) {
    cummulative_val += toReal(1.0 / 3.0) *
  ...
  if ((end % 2) == 0) {
    cummulative_val += toReal(0.5) * ((P[idx] * P[idx]) * h * r_mesh[idx] +
...
Real lsms::rel_eigenenergies(
  ...
  Real emax_init = 1.0e-6;
  Real emin_init = -10000000;

  if (l_qn != 0) {
    emin_init = pot[0] + (l_qn * (l_qn + 1)) / (2 * r_mesh[0] * r_mesh[0]);

    for (int ir = 1; ir < end; ir++) {
      emin_init =
          std::min(pot[ir] + l_qn * (l_qn + 1) / (toReal(2.0) * r_mesh[ir] * r_mesh[ir]), emin_init);
    }

    emin_init = emin_init * toReal(1.1);

  } else {
    emin_init = rel_energy_start(n_qn, k_qn, Z) * toReal(1.6);
  }

  if (e_init == 0) {
    e_init = -(Z * Z) / ((toReal(2.0) * n_qn * n_qn));
  } else if (e_init < emin_init || emax_init > 0) {
    e_init = emin_init / toReal(4.0);
  } else {
    e_init = e_init;
  }
  ...
  Real dE = 0.0;
  Real P_minus, P_plus, Q_minus, Q_plus;
  ...
  auto last_point = end;
  Real etol = energy_rel_tol;

  Real e_max = emax_init;
  Real e_min = emin_init;
  ...
  for (i = 0; i < iter_max; i++) {
    ...
    if (std::fabs(dE / E) < etol) {
      Real sq_norm = std::sqrt(norm_val);
...
Real lsms::nonrel_eigenenergies(
    Real Z, int n_qn, int l_qn, int k_qn, Real *dens, Real *p_array, Real *q_array,
    const Real *__restrict__ r_mesh, Real h,
    const Real *__restrict__ pot, std::size_t end, Real energy_rel_tol, int max_iter,
    Real e_init, int &converged, Real &delta_energy) {
  ...
  if (l_qn != 0) {
    ...
    emin_init = nrel_energy_start(n_qn, Z) * toReal(1.6);
  }

  
  if (e_init == 0) {
    e_init = -(Z * Z) / ((3 * n_qn * n_qn));
  } else if (e_init < emin_init || emax_init > 0) {
    e_init = emin_init / 3;
  } else {
    e_init = e_init;
...
Real lsms::rel_energy_start(int n, int k_qn, Real Z) {
...
Real lsms::nrel_energy_start(int n, Real Z) { return -Z * Z / (2 * n * n); }
