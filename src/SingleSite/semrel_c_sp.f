...
c     cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine semrel_sp(nrelv,clight,lmax,
     >                  energy,prel,pnrel,
     >                  matom,zlr,jlr,
     >                  h,jmt,jws,r,rv,
     >                  r_sph,
...
c     parameter (solver=1)
      ...
      parameter (nr=4)
c
c     real*4     phshft(iplmax+1)
      real*4     clight
      real*4     h,r_sph
      real*4     r(iprpts)
      real*4     rv(iprpts)
      real*4     coef(nr,iprpts)
c
      complex*8 energy
      complex*8 prel
      complex*8 pnrel
      complex*8 matom(0:lmax)
      complex*8 zlr(iprpts,0:lmax)
      complex*8 jlr(iprpts,0:lmax)
      complex*8 bjl(2,iprpts)
      complex*8 bnl(2,iprpts)
c$$$      complex*8 bes(2*(iplmax+1))
c$$$      complex*8 djl(iplmax+1)
c$$$      complex*8 dnl(iplmax+1)
      complex*8 bes(2*(lmax+1))
      complex*8 djl(lmax+1)
      complex*8 dnl(lmax+1)
      complex*8 g(iprpts)
      complex*8 f(iprpts)
      complex*8 gpl(iprpts)
      complex*8 fpl(iprpts)
      complex*8 pr
      complex*8 tandel
      complex*8 sqrtm1
c
      parameter (sqrtm1=(0.0d0,1.0d0))
      parameter (sname='semrel_sp')
c
c     ****************************************************************
...
c fit the potential to (A1*r+A2)/(1+A3*r)
      call fitpot_sp(r,rv,coef,nr,iend-1,iend)
c     call fitpot_sp(r,rv,coef,nr,506,iend)
c     do i=506,iend
c       do j=1,4
c         coef(j,i)=0.d0
c       enddo
...
c     call fitpot_sp(r,rv,coef,nr,506,iend)
c     call fitpot_sp(r,rv,coef,nr,jmt,iend)
      endif
c
c     ================================================================
...
c     ================================================================
      do l=0,lmax
        if(solver.eq.1) then
c        -------------------------------------------------------------
         call scalarr_sp(nrelv,clight,l,
c        call scalar_sp(nrelv,clight,l,
...
c    >               rv,r,h,506,iend,icmax,
     >               r_sph,iprint,istop)
         call scalari_sp(nrelv,clight,l,
     >               bjl,bnl,bes,bes(lmax+2),djl,dnl,g,f,gpl,fpl,
     >               tandel,matom(l),
     >               energy,prel,pnrel,
     >               rv,r,h,jmt,iend,icmax,
...
c        get j_l(r): used in constructing green function...........
         ...
         call rwave_sp(nrelv.eq.0,.true.,g,gpl,f,fpl,matom(l),l,energy,
