...
!
      subroutine single_scatterer_rel_sp(ce,psq,lmax,kmymax,
...
!      include 'atom_param.h'
      ...
      real*4 v0,vr,br,bopr,dx,rs,socsc
...
c
      ...
      complex*8 tminv2(2,2)
      complex*8 tminv(kmymax,kmymax)
      complex*8 gz2(iprpts,2,2),fz2(iprpts,2,2)
      complex*8 gj2(iprpts,2,2),fj2(iprpts,2,2)
!     complex*8 gz(ns,nuzp,kmymax),fz(ns,nuzp,kmymax)
!     complex*8 gj(ns,nuzp,kmymax),fj(ns,nuzp,kmymax)
      complex*8 gz(iprpts,nuzp,kmymax),fz(iprpts,nuzp,kmymax)
      complex*8 gj(iprpts,nuzp,kmymax),fj(iprpts,nuzp,kmymax)
      complex*8 fb(0:lmax+1),fn(0:lmax+1),fh(0:lmax+1)
      complex*8 fb1(0:lmax+1),fn1(0:lmax+1),fh1(0:lmax+1)
      complex*8 ce,psq,p,cevac,psqvac,pvac,sk,dk,xk,react,sqrtm1
      integer nuz,indz
      ...
      parameter(sname='single_scatterer_rel_sp')
c
c     write(6,'(a10)') idpot
...
!     kmymax=2*(lmax+1)*(lmax+1)
      call zeroout_sp(tminv,2*kmymax*kmymax)
c
!     idpot .eq. 0 -> Vacuum
      if(idpot.eq.0) then
c
        ...
        cevac=ce-v0
        pvac=csqrt(cevac)
        call csbf_sp(lmax+1,p,rs,fb,fn,fh)
        call csbf_sp(lmax+1,pvac,rs,fb1,fn1,fh1)
        do k=1,kmax
          l=k/2
          if(k.eq.2*l) then
            kap=l
...
!         write(6,*) sname,1
            ...
            call spzwafu_sp(socsc,ce,psq,l,my,vr,br,bopr,dx,ns,rs,
