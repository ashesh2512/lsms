...
int writeSingleAtomData_hdf5(hid_t loc_id, AtomData &atom, int present_atom)
{
  ...
  write_scalar<Real>(loc_id,"alat",atom.alat);
  write_scalar<Real>(loc_id,"Efermi",atom.efermi);
  write_scalar<Real>(loc_id,"Vdif",atom.vdif);
  write_scalar<Real>(loc_id,"Ztot",atom.ztotss);
  write_scalar<Real>(loc_id,"Zcore",atom.zcorss);
  write_scalar<Real>(loc_id,"Xstart",atom.xstart);
  write_scalar<Real>(loc_id,"rmt",atom.rmt);
  // printf("Wrote Scalars\n");
  write_vector<Real>(loc_id,"xvalws",&atom.xvalws[0],atom.nspin);
  ...
  for(int ns=0; ns<atom.nspin; ns++)
  {
    ...
    write_vector<Real>(loc_id,gname,&atom.vr(0,ns),atom.jmt);
    // printf("Wrote %s\n",gname);
    //return 0;
    snprintf(gname,100,"rhotot%1d",ns+1);
    write_vector<Real>(loc_id,gname,&atom.rhotot(0,ns),atom.jws);
    // printf("Wrote %s\n",gname);
    if(atom.numc>0) // write core states if they exist
    {
      snprintf(gname,100,"ec%1d",ns+1);
      write_vector<Real>(loc_id,gname,&atom.ec(0,ns),atom.numc);
      //printf("Wrote %s\n",gname);
    ...
    //printf("Wrote JTitle\n",gname);
    write_vector<Real>(loc_id,"evec",&atom.evec[0],3);
    //printf("Wrote evec\n",gname);
