      subroutine zsphbesj_sp(lmax,x,as,ac,x1,bj,bscrat)
c_______________________________________________________________________
c calculate spherical bessel functions j_l
c input: lmax integer scalar, max l for j_l
c        x    complex*8 scalar, argument of j_l
c returns: as complex*8 scalar, sin(x)
c          ac complex*8 scalar, cos(x)
c          x1 complex*8 scalar, 1/x
c          bj complex*8 array of (0:lmax), j_l
c          bscrat complex*8 array of (0:lmax), scratch space

c  xgz ornl 1994

      implicit complex*8(a-h,o-z)
      complex*8 bj(0:lmax),bscrat(0:lmax)

      as=sin(x)
      ac=cos(x)
      x1=1.0d0/x
      bj(0)=as*x1
c   Forward recursion for small L
    1 k=ishft(int(0.75d0*(abs(real(x))+abs(imag(x))))+2,-2)
      bscrat(0)=as
      if(k.ge.1) then
        if(k.gt.lmax) k=lmax
        bscrat(1)=bj(0)-ac
        bj(1)=bscrat(1)*x1
        do l=2,k
          bscrat(l)=(2*l-1)*bj(l-1)-bscrat(l-2)
          bj(l)=bscrat(l)*x1
        end do
        if(k.eq.lmax) return
      endif
c   Backward recursion from very large L down to L=k
      a=bscrat(k)
      nm=ishft(lmax,4)
      aj=2*nm+3
      x2=x*x
      do l=nm,lmax+2,-1
         aj=(2*l+1)-x2/aj
      end do
      bscrat(lmax)=(2*lmax+3)*aj*x1-x
      bj(lmax)=bscrat(lmax)*x1
      bscrat(lmax-1)=(2*lmax+1)*bj(lmax)-aj
      do l=lmax-1,k+1,-1
         bj(l)=bscrat(l)*x1
         bscrat(l-1)=(2*l+1)*bj(l)-bscrat(l+1)
      end do
c scale to give correct bj(k)
      aj=a/bscrat(k)
      do l=k+1,lmax
        bj(l)=aj*bj(l)
      end do
      return
      end