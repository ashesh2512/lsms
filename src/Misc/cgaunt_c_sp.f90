...
subroutine cgaunt_sp(lmax, clm, &
   ...
   !
   real*4    plmg((2 * lmax + 1) * (2 * lmax + 2) / 2, 2 * lmax + 1)
   real*4    clm((2 * lmax + 1) * (2 * lmax + 2) / 2)
   real*4    tg(2 * (2 * lmax + 1))
   real*4    wg(2 * (2 * lmax + 1))
   real*4    gaunt_sp
   real*4    cgnt(lmax + 1, (lmax + 1)**2, (lmax + 1)**2)
   real*4    ctemp
   real*4    ctol
   !
   parameter (ctol = 1.0-7)
   parameter (sname = 'gaunt_sp')
   !
   !     ****************************************************************
   !     generate:
   !
   ...
   !     ----------------------------------------------------------------   
   call gauss_legendre_points_sp(-1.0, 1.0, tg, wg, 2 * (2 * lmax + 1))
   !     ----------------------------------------------------------------
   !
   !     ================================================================
   ...
   do ng = 1, n1
      ...
      call plm_normalized_sp(2 * lmax, tg(ng), plmg(1, ng))
   enddo
   !
   if(iprint.ge.1) then
   ...
   do j2 = 1, n2
      ...
      do j1 = 1, n1
         ...
         do l3 = l1 + l2, max(abs(m3), abs(l1 - l2)), -2
            ctemp = gaunt_sp(l1, m1, l2, m2, l3, m3, 2 * lmax, 2 * lmax + 1, &
               wg, plmg, clm)
